...
root = Tk()
root.title('I am a top level, parent to other widgets')

#create aframe widget for palcing menu
my_menu_bar = Frame(root, relief=RAISED, bd=2)
my_menu_bar.pack(fill = X)

#Create menu widget 1 and sub menu1
my_menu_button = Menubutton(my_menu_bar, text = 'Menu Button 1', )
...
#Create menu widget 2 and sub menu2
my_menu_button_2 = Menubutton(my_menu_bar, text = 'Menu 2', )
my_menu_button_2.pack(side = LEFT)

#MENU Widget
my_menu_2 = Menu(my_menu_button_2, tearoff = 0)
my_menu_21 = Menu(my_menu_button_2, tearoff = 0)
my_menu_22 = Menu(my_menu_button_2, tearoff = 0)
my_menu_button_2['menu'] = my_menu_2
...
#create another frame(my_frame_2) to hold a list box, spoinbox widget, sale widgets
...
dance_photo = PhotoImage(file='tenor.gif')
...
Label(my_frame_2, text='Below is the example of my_listbos widget:').pack()
my_listbox = Listbox(my_frame_2, height=4)
for line in ['Listbox Choice 1','Choise 2','Choice 3', 'Choice 4']:
	my_listbox.insert(END, line)
my_listbox.pack()

#spinbox widget
Label(my_frame_2, text='Below is an example of spinbox widget').pack()
Spinbox(my_frame_2, value=(1, 2, 4, 8, 10)).pack()
...
my_frame_3 = Frame(root, bd=2, relief=SUNKEN).pack()
...
#****PLEASE THIS ONE IS NOT WORKING VERY WELL EXPECIALLY LAST LINE.
#ADD SCROLLBAR WIDGET TO THE TEXT WIDGET
my_scrollbar = Scrollbar = Scrollbar(my_frame_3, orient=VERTICAL, command=my_text.yview)
my_scrollbar.pack()

my_text.configure(yscrollcommand = my_scrollbar.set)
#my_frame_3.pack()  #****PLEASE THIS ONE IS NOT WORKING VERY WELL
...
#my_canvas.create_oval(30,15,60,60, fill='blue')
...
left_paned_text = Text(my_paned_window_1, height=6, width=15)
my_paned_window_1.add(left_paned_text)
